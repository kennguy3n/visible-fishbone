...
use crate::traffic_gen::{
    ...
    RawSocketGenerator, Subnet, TrafficError, TrafficGenerator,
};
...
/// The RFC-2544-style benchmarking subnets the single-stream `throughput`
/// bench uses, kept in lockstep so the wire scaling floor is the same
/// workload as the existing single-stream number.
fn sampler_for(ip_version: IpVersion, seed: u64) -> Result<FiveTupleSampler, TrafficError> {
    ...
    FiveTupleSampler::new(src, dst, (1024, 65_535), (1, 1024), seed)
}
...
fn builder_for(config: &WireScalingConfig, seed: u64) -> Result<PacketBuilder, TrafficError> {
    let sampler = sampler_for(config.ip_version, seed)?;
...
/// one worker thread per generator that — released together by a barrier —
/// transmits flat-out for `duration`. Returns the per-stream outcomes in
/// ascending stream-index order.
///
/// Every generator is constructed up front, on this thread, before any
/// worker spawns: socket construction is untimed setup, and — crucially —
/// a failure (e.g. `EPERM` without `CAP_NET_RAW`) aborts cleanly here
/// rather than leaving N−1 workers blocked forever on a barrier a panicked
    ...
    let handles: Vec<_> = generators
        ...
        .map(|(q, mut emitter)| {
            ...
            thread::Builder::new()
                ...
                .spawn(move || {
                    // Release every stream into its timed loop together so
                    // the streams genuinely contend for the whole window.
                    barrier.wait();
                ...
                .expect("spawn wire multi-queue worker thread")
        })
        .collect();

    let mut outcomes: Vec<StreamOutcome> = Vec::with_capacity(queues);
