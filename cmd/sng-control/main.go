...
func run() error {
	...
	router, webhookWorker, integrationWorker, appRegHandler, appSyncer, policySimHandler, aiSvc, evidenceScheduler, err := buildRouter(&cfg, logger, pool, health, telReplay, telPublisher, mx)
	if err != nil {
...
func buildRouter(
	...
	mx *metrics.Metrics,
) (http.Handler, *webhook.DeliveryWorker, *integration.DeliveryWorker, *handler.AppRegistryHandler, *appdb.Syncer, *handler.PolicySimulationHandler, *aisvc.Service, *compliance.Scheduler, error) {
	...
	if err != nil {
		return nil, nil, nil, nil, nil, nil, nil, nil, fmt.Errorf("compliance evidence automation: %w", err)
	}
	...
	return router, webhookWorker, integrationWorker, appRegHandler, appSyncer, policySimHandler, aiSvc, evidenceScheduler, nil
}
