...
//!
//! Endpoint-tier sibling of [`sng_edge::subsystems::comms`].
//! Pulls policy bundles targetted at
//! [`sng_policy_eval::BundleTarget::Endpoint`] (not `Edge`) and
//! shares its underlying [`ControlPlaneClient`] with the
//! telemetry subsystem so the agent maintains exactly one TLS
//! config + endpoint pairing.
//!
//! The start task:
//!
//! 1. Establishes a fresh [`ControlPlaneConnection`] via
//!    [`ControlPlaneClient::connect`].
//! 2. Pulls a bundle at the operator-configured cadence.
//! 3. Publishes any new bundle bytes through the supplied
//!    callback so the policy_eval subsystem can hot-swap its
//!    compiled state.
...
/// downstream consumers. Wrapped in a `Send + Sync` Arc so the
/// start task can hand off without re-cloning the trait object
/// for each callback fire.
pub type BundlePublisher =
    Arc<dyn Fn(BundleTarget, Vec<u8>) -> Result<(), String> + Send + Sync + 'static>;

/// Endpoint-tier control-plane comms subsystem.
...
pub struct CommsStats {
    ...
    pub pulls: AtomicU64,
    /// Pulls that surfaced a fresh, validated bundle.
...
pub enum CommsBuildError {
    ...
    /// rustls ClientConfig construction failed.
            ...
            loop {
                tokio::select! {
                    ...
                    _ = ticker.tick() => {
                        ...
                        // populated `conn` or hit `continue`. A panic here
                        // would be a logic bug; match instead of
                        // `.expect` so a future refactor that breaks the
                        // invariant skips the pull rather than aborting.
        ...
        SubsystemHealth {
            ...
            detail: Some(format!(
                "connects={connects}, connect_failures={connect_failures}, \
                 pulls={pulls}, fresh={pulls_fresh}, not_modified={pulls_not_modified}, \
...
/// stripping the IPv6 brackets if present. Used as the SNI
/// when the operator didn't override `server_name`.
...
/// `-----END CERTIFICATE-----` marker so a multi-cert PEM
/// bundle is admitted without pulling in `rustls-pemfile`.
fn load_root_pems(
    path: &Path,
