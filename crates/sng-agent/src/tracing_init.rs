...
//!
//! 1. `--log-filter` CLI flag (also accepts `SNG_AGENT_LOG_FILTER`
//!    env var via clap).
//! 2. `RUST_LOG` env var (canonical tracing-subscriber path).
//! 3. The compiled-in default: `info,sng_agent=info`.
//!
//! Init is idempotent — the integration tests in `tests/` can
//! call it once per process without tripping the "global
//! default already set" panic.
    ...
    INIT.get_or_init(|| {
        ...
        let filter = match EnvFilter::try_new(&filter_str) {
            ...
            Err(e) => {
                ...
                EnvFilter::new("info,sng_agent=info")
            }
        };
        if cli.log_json {
...
mod tests {
    ...
    fn pick_filter_directive_falls_back_to_default() {
        ...
        // and `cargo test` runs this test binary single-
        // threaded by default.
        #[allow(unsafe_code)]
        unsafe {
        ...
        assert_eq!(got, "info,sng_agent=info");
    }
}
